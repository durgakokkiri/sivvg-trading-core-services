...
public class AuthControllerTest {
	...
	public void registerUser_success() throws Exception {
		...
		RegisterResponse response = new RegisterResponse("USR123", "User registered successfully");

		when(userService.registerNewUser(any(RegisterRequest.class))).thenReturn(response);

		mockMvc.perform(post("/api/v1/auth/user-register").contentType(MediaType.APPLICATION_JSON)
	...
	public void login_success() throws Exception {
		...
		when(authService.processLoginAndSendOtp(any(LoginRequest.class))).thenReturn(new LoginResponse("OTP Sent"));
	...
	public void verifyOtp_success() throws Exception {
		...
		when(authService.verifyOtpAndReturnToken("USR123", "123456")).thenReturn("jwt-token");

		mockMvc.perform(post("/api/v1/auth/verify-otp").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(request))).andExpect(status().isOk())
	...
	public void forgotSendOtp_success() throws Exception {
		...
		when(authService.sendForgotOtp("test@gmail.com")).thenReturn("USR123"); // <-- this will be returned by
																				// controller

		mockMvc.perform(post("/api/v1/auth/forgot/send-otp").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(request))).andExpect(status().isOk())
	...
	public void resendOtp_success() throws Exception {
		...
		mockMvc.perform(post("/api/v1/auth/resend-otp").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(request))).andExpect(status().isOk())
	...
	public void resetPassword_success() throws Exception {
		...
		mockMvc.perform(post("/api/v1/auth/forgot/reset-password").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(request))).andExpect(status().isOk())
